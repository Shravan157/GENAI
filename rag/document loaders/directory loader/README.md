...
## What is this?
This folder loads multiple PDF files from a directory. DirectoryLoader finds files, while PyPDFLoader reads each PDF. In short: it batch-loads documents from a folder.

## Why does it exist?
- Use when you have many files of the same type to process.
...
### Files
- `directory_loader.py`: Loads multiple PDFs from a directory using DirectoryLoader and PyPDFLoader.
...
This folder loads multiple PDF files from a directory. DirectoryLoader finds files, while PyPDFLoader reads each PDF.
